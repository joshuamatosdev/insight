      ...
      <CardBody>
        ...
        >
          {data.length > 0 && (
            <>
              <Text variant="caption" color="secondary" style={{ fontSize: '10px' }}>
                {new Date(data.at(0)?.date ?? '').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
              </Text>
              {data.length > 2 && (
                <Text variant="caption" color="secondary" style={{ fontSize: '10px' }}>
                  {new Date(data.at(Math.floor(data.length / 2))?.date ?? '').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </Text>
              )}
              <Text variant="caption" color="secondary" style={{ fontSize: '10px' }}>
                {new Date(data.at(-1)?.date ?? '').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
              </Text>
            </>
          )}
        </Box>
