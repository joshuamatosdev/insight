// Tailwind Plus UI Blocks - Pricing
...
// =============================================================================
// 1. Two tiers with emphasized right tier (Light)
// =============================================================================
export function PricingTwoTiersLight() {
  return (
    <Box className="relative isolate bg-white px-6 py-24 sm:py-32 lg:px-8">
      <Box aria-hidden="true" className="absolute inset-x-0 -top-3 -z-10 transform-gpu overflow-hidden px-36 blur-3xl">
        <Box
          style={{
            clipPath:
      ...
      <Grid columns={1} className="mx-auto mt-16 max-w-lg items-center gap-y-6 sm:mt-20 sm:gap-y-0 lg:max-w-4xl lg:grid-cols-2">
        {tiers.map((tier, tierIdx) => (
          ...
          >
            <h3
              id={tier.id}
              className={clsx(tier.featured ? 'text-indigo-400' : 'text-indigo-600', 'text-base/7 font-semibold')}
            >
              {tier.name}
            </h3>
            <Flex align="baseline" className="mt-4 gap-x-2">
              <span
                className={clsx(
...
// =============================================================================
// 2. Two tiers with emphasized right tier (Dark)
// =============================================================================
export function PricingTwoTiersDark() {
  return (
    <Box className="relative isolate bg-gray-900 px-6 py-24 sm:py-32 lg:px-8">
      <Box aria-hidden="true" className="absolute inset-x-0 -top-3 -z-10 transform-gpu overflow-hidden px-36 blur-3xl">
        <Box
          style={{
            clipPath:
      ...
      <Grid columns={1} className="mx-auto mt-16 max-w-lg items-center gap-y-6 sm:mt-20 sm:gap-y-0 lg:max-w-4xl lg:grid-cols-2">
        {tiers.map((tier, tierIdx) => (
          ...
          >
            <h3
              id={tier.id}
              className={clsx(tier.featured ? 'text-indigo-400' : 'text-indigo-400', 'text-base/7 font-semibold')}
            >
              {tier.name}
            </h3>
            <Flex align="baseline" className="mt-4 gap-x-2">
              ...
              <span className={clsx(tier.featured ? 'text-gray-400' : 'text-gray-400', 'text-base')}>/month</span>
            </Flex>
            <p className={clsx(tier.featured ? 'text-gray-300' : 'text-gray-300', 'mt-6 text-base/7')}>
              {tier.description}
            </p>
            <ul
              role="list"
            ...
            >
              {tier.features.map((feature) => (
                <li key={feature} className="flex gap-x-3">
                  <CheckIcon
                    aria-hidden="true"
                    className={clsx(tier.featured ? 'text-indigo-400' : 'text-indigo-400', 'h-6 w-5 flex-none')}
                  />
                  {feature}
                </li>
              ))}
...
export default PricingTwoTiersLight
