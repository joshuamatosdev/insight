...
export function OAuthCallbackPage(): React.ReactElement {
    ...
    const searchParams = useSearch({from: '/_public/oauth/callback'});
    ...
    useEffect(() => {
        ...
        if (error !== undefined && error !== null) {
            setStatus('error');
            setErrorMessage(searchParams.error_description ?? 'Authentication was cancelled or failed');
            return;
        }

        if (provider === undefined || provider === null || code === undefined || code === null) {
            setStatus('error');
        ...
        const processCallback = async () => {
            try {
                ...
                const response = await processOAuthCallback({
                    provider,
                    providerUserId: searchParams.user_id ?? '',
                    email: searchParams.email ?? '',
                    firstName: searchParams.first_name ?? undefined,
                    lastName: searchParams.last_name ?? undefined,
                    accessToken: searchParams.access_token ?? undefined,
                    refreshToken: searchParams.refresh_token ?? undefined,
                });
